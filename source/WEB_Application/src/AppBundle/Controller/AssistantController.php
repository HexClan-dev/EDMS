...
class AssistantController extends Controller
{
    ...
    public function reportStudentAction()
    {
        $em = $this->getDoctrine()->getManager();
        $user = $this->getUser();
        $students = $em->getRepository('AppBundle:Person')->getacceptedStudents();
        $leftSDB = $this->getLeftSideMenu($em);
    ...
    public function performanceAction()
    {
        $em = $this->getDoctrine()->getManager();
        $user = $this->getUser();
        $myPerformance = $em->getRepository('AppBundle:Performances')->getAllStudentPerformances();
        $leftSDB = $this->getLeftSideMenu($em);
    ...
    /**
     * @Route("/assistant/performance/new/{student_}", name="new_performance")
     */
    public function createPerformanceAction(Request $request, Person $student_)
    {
        $em = $this->getDoctrine()->getManager();
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            $performance->setStudent($student_);
...
//     */
    public function editProfileAction(Request $request){
        ...
        if($form->isSubmitted() && $form->isValid()){
            ...
            return $this->redirectToRoute('show_profile_assistant');
    ...
    /**
     * @Route("/assistant/report/technical/problem", name="report_tech_problem")
     */
    public function reportTechProblemAction()
    {
        ...
        $pending_problems = $em->getRepository('AppBundle:TechProblems')->showALLPending_problems_assistant();
    ...
    /**
     * @Route("/assistant/report/technical/problem", name="tech_problems_assistant")
     * @return Response
     */
    ...
    /**
     * @Route("/assistant/technical/problem/confirm/{techProblems}", name="approve_problem")
     */
    public function fixedTechProblemAction(TechProblems $techProblems){


        if($techProblems->isConfirmed())
            $techProblems->setIsConfirmed(false);
        else $techProblems->setIsConfirmed(true);


        $em = $this->getDoctrine()->getManager();

        $em->persist($techProblems);
        $em->flush();


        return $this->redirectToRoute('tech_problems_assistant');
    ...
    public function getLeftSideMenu(EntityManager $em){
        ...
        /** @var Person $user */
        $user=$this->getUser();
        $cnt_members = $em->getRepository('AppBundle:Person')->getAllTheStudentMembersOFDorm();
        ...
        return array("members"=>count($cnt_members),'events'=>$events,'tech'=>$cnt_tech,'performance'=>$cnt_performances);
