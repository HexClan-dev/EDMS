...
//    /**
//     * @Route("/student/event/participating/{ventParticiating}", name="invited_events")
//     */
//
...
//        $user =  $em->getRepository('AppBundle:Person')->getPerson($user->getEmail(),$user->getCardId());
        ...
        if($form->isSubmitted() && $form->isValid()){
            ...
            try {
                ...
                $this->addFlash('failure','The profile was not updated!! \t'.$e->getMessage());
		}
    ...
    public function acceptInvitation(EventParticipants $ventParticiating){

        if($ventParticiating->getisParticipating()){
            $ventParticiating->setIsParticipating(false);
        }else{
            $ventParticiating->setIsParticipating(true);
        }

        $em =  $this->getDoctrine()->getManager();
        $em->persist($ventParticiating);
        $em->flush();
